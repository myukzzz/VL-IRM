...
print(f"Parse COCO caption with {args.sg_parser} SG parser")
if args.sg_parser == 'java':
    processed_coco_caption_file = 'DATASET/VG150/weak_supervisions/language_supervised-coco_caption-grounding=glipL-java.json'
    ...
    processed_coco_caption_file = 'DATASET/VG150/weak_supervisions/language_supervised-coco_caption-grounding=glipL-python.json'
...
if os.path.exists(processed_coco_caption_file):
    ...
    for cap in tqdm(coco_captions['annotations']):
        ...
        ####### java sg parser ########
        if args.sg_parser == 'java':
            cap_graph = json.loads(cap['text_scene_graph']) # load java parsed sg
            for e in cap_graph['objects']:
                caption_object_vocabs.add(e['names'][0])
            for r in cap_graph['relationships']:
                triplet = tuple(r['text'])
                caption_relation_vocabs.add(triplet[1])
        ...
        ####### python sg parser ########
            ...
            cap_graph = sng_parser.parse(cap['caption']) # python sg parser text parsing
...
print(f"image_count/no_triplet_image_count = {len(coco_parsing_info['text_scene_graph'])}/{len([k for k,v in coco_parsing_info['text_scene_graph'].items() if len(v['triplets'])==0])}")
print(f"triplet_count = {sum([len(v['triplets']) for k,v in coco_parsing_info['text_scene_graph'].items()])}")
print(f"caption_object_vocabs_to_vg_objs mapped/all = {len([v for k,v in coco_parsing_info['caption_object_vocabs_to_vg_objs'].items() if len(v)>0])}/{len(coco_parsing_info['caption_object_vocabs_to_vg_objs'])}")
print(f"caption_relation_vocabs_to_vg_rels mapped/all = {len([v for k,v in coco_parsing_info['caption_relation_vocabs_to_vg_rels'].items() if len(v)>0])}/{len(coco_parsing_info['caption_relation_vocabs_to_vg_rels'])}\n\n")
