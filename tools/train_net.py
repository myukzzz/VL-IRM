...
from maskrcnn_benchmark.utils.logger import setup_logger, print_params
...
def train(cfg, local_rank, distributed, use_tensorboard=False,):
    ...
    train_ov_relation = cfg.SOLVER.ov_relation
    test_ov_relation = cfg.SOLVER.ov_relation
    more_base_rel = cfg.SOLVER.more_base_rel
    ##############################################################
    print("train_ov_relation:",train_ov_relation)
    print("test_ov_relation:", test_ov_relation)
    print("more_base_rel:", more_base_rel)
    ...
    ###################logger###############
    sys.stdout = Tee(os.path.join(cfg.OUTPUT_DIR,cfg.OUTPUT_log+'_out_log.txt'))
    sys.stderr = Tee(os.path.join(cfg.OUTPUT_DIR,cfg.OUTPUT_log+'_err_log.txt'))
    ...
    extra_checkpoint_data = checkpointer.load(try_to_find(cfg.MODEL.WEIGHT),skip_optimizer=not use_train)
    ...
    use_CLIPtext=cfg.SOLVER.use_CLIPtext
    do_train(
        cfg,
        model,
        data_loader,
        ...
        data_loaders_val,
        meters,
        train_ov_relation=True,
        test_ov_relation=True,
