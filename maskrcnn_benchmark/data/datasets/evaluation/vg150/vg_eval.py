...
from maskrcnn_benchmark.data.datasets.evaluation.vg150.sgg_eval import SGRecall, SGNoGraphConstraintRecall, SGZeroShotRecall, SGNGZeroShotRecall, SGPairAccuracy, SGMeanRecall, SGNGMeanRecall, SGAccumulateRecall
...
def do_vg_evaluation(
    ...
    use_ov_relation=cfg.SOLVER.ov_relation
    more_base_rel = cfg.SOLVER.more_base_rel
    ...
    for image_id, prediction in enumerate(predictions):
        ...
        if not cfg.MODEL.RELATION_ON or not cfg.MODEL.DYHEAD.RELATION_CONSISTENCY_ON and not use_topkobject:####################
    ...
    if "bbox" in iou_types:
        ...
        for image_id, prediction in enumerate(predictions):
            if use_topkobject:
                box = convert_to_xywh(prediction.get_field('topk_boxes')).detach().cpu().numpy()
            else:
    ...
    if "relations" in iou_types or cfg.MODEL.DYHEAD.RELATION_CONSISTENCY_ON:################初始化
        result_dict = {}
        evaluator = {}
        ...
        eval_zeroshot_recall = SGZeroShotRecall(result_dict,num_rel_category,dataset.ind_to_predicates)
        ...
        for groundtruth, prediction in zip(groundtruths, predictions):
            evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator,use_ov_relation=use_ov_relation,use_topkobject=use_topkobject,more_base_rel=more_base_rel)
        
        # calculate mean recall
        eval_mean_recall.calculate_mean_recall(mode)
        eval_ng_mean_recall.calculate_mean_recall(mode)
        eval_zeroshot_recall.calculate_mean_recall(mode)
...
def evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator,use_ov_relation,use_topkobject,more_base_rel=False):
    """
    Returns:
    ...
    local_container['rel_scores_topk'] = prediction.get_field('pred_rel_scores_topk').detach().cpu().numpy()
    local_container['rel_cls_topk'] = prediction.get_field('pred_rel_cls_topk').detach().cpu().numpy()
    local_container['pred_rel_inds_topk'] = prediction.get_field('rel_pair_idxs_topk').long().detach().cpu().numpy()
    # (#pred_rels, num_pred_class)

    # about objects
    if use_topkobject:
        local_container['pred_boxes'] = prediction.get_field('topk_boxes').detach().cpu().numpy()
    else:
    ...
    evaluator['eval_zeroshot_recall'].prepare_zeroshot(global_container, local_container,use_ov_relation=use_ov_relation,more_base_rel=more_base_rel)
    ...
    evaluator['eval_zeroshot_recall'].collect_mean_recall_items(global_container, local_container, mode,more_base_rel=more_base_rel)
    # Mean Recall
