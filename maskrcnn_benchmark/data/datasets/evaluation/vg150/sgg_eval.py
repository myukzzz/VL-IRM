...
class SGRecall(SceneGraphEvaluation):
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        if self.usetopk:
            pred_scores = local_container['rel_scores_topk']
            #rel_cls_topk = local_container['rel_cls_topk']
            #rel_pair_topk = local_container['pred_rel_inds_topk']
            pred_rels = np.concatenate([local_container['pred_rel_inds_topk'],local_container['rel_cls_topk'][:,None]],axis=1)
        else:
...
class SGNoGraphConstraintRecall(SceneGraphEvaluation):
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        obj_scores_per_rel = obj_scores[pred_rel_inds].prod(1)####计算乘积
...
class SGZeroShotRecall(SceneGraphEvaluation):
    def __init__(self, result_dict,num_rel,ind_to_predicates):
        super(SGZeroShotRecall, self).__init__(result_dict)
        self.novel_count = 0
        self.all_count = 0
        self.ov_relation=True
        self.num_rel=num_rel
        self.rel_name_list = ind_to_predicates[1:]
        self.OIV6 = False
        print("OIV6:",self.OIV6)
        if self.ov_relation:
            if self.OIV6:
                #self.base_rel_idx = [1, 2, 3, 7, 8, 13, 17, 18, 20]
                self.base_rel_idx_ori = [26, 24, 4, 18, 21, 9, 13, 20, 28, 8, 16, 1, 7, 2, 0]
                ...
                self.base_rel_idx = [i + 1 for i in self.base_rel_idx_ori]
                self.unseen_rels = [9, 36, 38, 45, 22, 30, 34, 37, 16, 46, 39, 7, 14, 31, 49]
                self.unseen_rels_onlyrel = [unseen_rel - 1 for unseen_rel in self.unseen_rels]
                self.base_rels_new = list(set(self.rand_relation_idx.tolist()) - set(self.unseen_rels_onlyrel))
                self.base_rel_idx_new = [i + 1 for i in  self.base_rels_new]
    ...
    def register_container(self, mode):
        ...
        self.result_dict[mode + '_zeroshot_mean_recall'] = {20: 0.0, 50: 0.0, 100: 0.0}
        self.result_dict[mode + '_zeroshot_recall_collect'] = {20: [[] for i in range(self.num_rel)], 50: [[] for i in range(self.num_rel)], 100: [[] for i in range(self.num_rel)]}
        self.result_dict[mode + '_zeroshot_recall_list'] = {20: [], 50: [], 100: []}
    ...
    def prepare_zeroshot(self, global_container, local_container,use_ov_relation=False,more_base_rel=False):
        gt_rels = local_container['gt_rels']
        gt_classes = local_container['gt_classes']
        ...
        unseen_obj_cats = [9, 10, 11, 20, 22, 23, 26, 27, 28, 37, 38, 40, 41, 45, 52, 53, 54, 57, 58, 60, 61, 64, 66, 74, 78, 87, 91, 95, 97, 99, 111, 112, 113, 115, 121, 124, 126, 127, 134, 135, 136, 144, 145, 149, 150]####45
        ##############my zR#######################
        if use_ov_relation:
    ...
    def collect_mean_recall_items(self, global_container, local_container, mode,more_base_rel=False):
        pred_to_gt = local_container['pred_to_gt']
        gt_rels = local_container['gt_rels']
        if more_base_rel:
            self.base_rel_idx = self.base_rel_idx_new


        for k in self.result_dict[mode + '_zeroshot_recall_collect']:
            # Zero Shot Recall
            match = reduce(np.union1d, pred_to_gt[:k])
            ...
            if len(self.zeroshot_idx) > 0:
                ...
                for idx in range(gt_rels.shape[0]):
                    ...
                    if local_label not in self.base_rel_idx:
                ...
                for n in range(self.num_rel):
                    if recall_count[n] > 0:
                        self.result_dict[mode + '_zeroshot_recall_collect'][k][n].append(float(recall_hit[n] / recall_count[n]))
    ...
    def calculate_mean_recall(self, mode):
        for k, v in self.result_dict[mode + '_zeroshot_mean_recall'].items():
            ...
            for idx in range(num_rel_no_bg):
                if len(self.result_dict[mode + '_zeroshot_recall_collect'][k][idx+1]) == 0:
                    tmp_recall = 0.0
                else:
                    tmp_recall = np.mean(self.result_dict[mode + '_zeroshot_recall_collect'][k][idx+1])####
                self.result_dict[mode + '_zeroshot_recall_list'][k].append(tmp_recall)
                sum_recall += tmp_recall
            if not self.OIV6:
                self.result_dict[mode + '_zeroshot_mean_recall'][k] = sum_recall / float(len(self.unseen_rels))
            else:
                self.result_dict[mode + '_zeroshot_mean_recall'][k] = sum_recall / (float(num_rel_no_bg)-float(len(self.base_rel_idx)))
        return
