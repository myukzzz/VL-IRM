...
def convert_od_to_grounding_simple(
    ...
    def generate_sentence_from_labels(positive_label_list, negative_label_list, disable_shuffle=True,positive_rel_label_list=None):
        label_to_positions = {}

        rel_label_to_positions = {}

        label_list = negative_label_list + positive_label_list#1：105
        if not disable_shuffle:#false
        ...
        for index, label in enumerate(label_list):
            ...
            if caption_prompt is not None:#false
                pheso_caption += clean_name(caption_prompt[index]['name'])
            else:#########################
                pheso_caption += clean_name(ind_to_class[label])  # NOTE: slight change...  加入label的str
            ...
            label_to_positions[label] = [start_index, end_index]########记录label的str的位置

            if index != len(label_list) - 1:
                pheso_caption += separation_tokens


        for idx,relabel in enumerate(positive_rel_label_list):
            start_index = len(predicates_caption)
            predicates_caption += clean_name(ind_to_predicates[relabel])
            end_index = len(predicates_caption)

            rel_label_to_positions[relabel] = [start_index, end_index]########记录label的str的位置

            if idx != len(positive_rel_label_list) - 1:
                predicates_caption += separation_tokens

        return label_to_positions, pheso_caption, rel_label_to_positions,predicates_caption

    label_list = list(sorted(ind_to_class.keys()))  # do not include the background

    rel_label_list = list(sorted(ind_to_predicates.keys()))

    label_to_positions, pheso_caption,rel_label_to_positions,predicates_caption = generate_sentence_from_labels(
        positive_label_list=label_list,
        negative_label_list=[],
        disable_shuffle=disable_shuffle,#true
        positive_rel_label_list = rel_label_list,
    )
    ...
    '''
    areas = target.area()#面积
    greenlight_span_for_masked_lm_objective = []
    for i in range(len(target)):#box个数
        new_target_i = {}
    ...
    return new_target, pheso_caption, greenlight_span_for_masked_lm_objective,rel_label_to_positions,predicates_caption

def convert_od_to_grounding_with_pseudo_triplet_caption(target, ind_to_classes, ind_to_predicates):
    pseudo_caption = ""
    instance_to_positions = {}
    obj_labels = target.get_field("labels").tolist()

    # relations
    relations = target.get_field("relation")
    for (s, o) in relations.nonzero():
        p_name = ind_to_predicates[relations[s, o]]
        subj_name = ind_to_classes[obj_labels[s]]
        obj_name = ind_to_classes[obj_labels[o]]
    ...
    for inst_id in range(len(target)):
        # add other instances with no relations
        if inst_id not in instance_to_positions:
            obj_name = ind_to_classes[obj_labels[inst_id]]
            obj_text = f"{obj_name}. "
            pseudo_caption += obj_text
            instance_to_positions[inst_id] = [[len(pseudo_caption)-len(obj_name)-2, len(pseudo_caption)-2]]

        new_target_i = {}
        new_target_i["area"] = areas[inst_id]
        new_target_i["iscrowd"] = 0
        new_target_i["category_id"] = target.extra_fields["labels"][inst_id].item()
        new_target_i['bbox'] = target.bbox[inst_id].numpy().tolist()
