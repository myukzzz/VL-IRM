...
from .datasets.visual_genome150 import OIV6

def build_dataset(cfg, dataset_list, transforms, dataset_catalog, is_train=True, class_concat=False, extra_args={},train_ov_relation=None,more_base_rel=False):
    """
    Arguments:
    ...
    for dataset_id, dataset_name in enumerate(dataset_list, 1):
        if dataset_name == "sgg_oiv6":
            image_set = 'train' if is_train else 'test'
            dataset = OIV6(tokenizer=extra_args['tokenizer'],image_set=image_set,transforms=transforms)
        else:
            if is_train:
            ...
            factory = getattr(D, data["factory"])#VG150Dataset
            args = data["args"]
            ...
            if copy != -1:
                ...
                dataset = factory(**args,train_ov_relation=train_ov_relation,more_base_rel=more_base_rel)###########创建数据集
...
def make_batch_data_sampler(
    ...
    if aspect_grouping:#############
        ...
        aspect_ratios = _compute_aspect_ratios(dataset)#每个样本的ratio——长宽比
...
def make_data_loader(cfg, is_train=True, is_distributed=False, num_replicas=None, rank=None, start_iter=0,train_ov_relation=None,more_base_rel=False):
    ...
    DatasetCatalog = paths_catalog.DatasetCatalog###config
    if len(cfg.DATASETS.REGISTER) > 0:#false
    ...
    dataset_list = cfg.DATASETS.TRAIN if is_train else cfg.DATASETS.TEST#vg150_train
    ...
    if isinstance(dataset_list[0], (tuple, list)):
        ...
        datasets = build_dataset(cfg, dataset_list, transforms, DatasetCatalog, is_train,
                                 class_concat=cfg.DATASETS.CLASS_CONCAT,
                                 extra_args=extra_args,train_ov_relation=train_ov_relation,more_base_rel=more_base_rel)
    ...
    for di, dataset in enumerate(datasets):
        if is_train and cfg.SOLVER.MAX_EPOCH > 0:###################
            ...
            cfg.defrost()
            cfg.SOLVER.MAX_ITER = num_iters#78145
        ...
        if is_train and cfg.DATALOADER.DISTRIBUTE_CHUNK_AMONG_NODE:#false
    ...
    if is_train and cfg.SOLVER.MULTI_MAX_EPOCH:#false
        cfg.defrost()
        cfg.SOLVER.MULTI_MAX_ITER += (
            cfg.SOLVER.MULTI_MAX_EPOCH[-1] * min([len(dataset) // cfg.SOLVER.IMS_PER_BATCH for dataset in datasets]),)
        cfg.freeze()

    if is_train and not cfg.DATASETS.ALTERNATIVE_TRAINING and not cfg.DATASETS.MULTISTAGE_TRAINING:#false
