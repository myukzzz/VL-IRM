...
class GeneralizedVLRCNN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.hoi_prefix = nn.Parameter(torch.empty(8, 768))
    ...
    def encode_rel_text(self,texts,device):
        all_token_embeddings = []
        all_masks = []
        eot_indices = []
        for rel in texts:
            relidx = torch.Tensor(self.tokenizer.encode(rel)).to(device)  # 101 start 102 end

            remain_length = self.context_length - self.prefix_length - len(relidx)
            rel_mask = torch.cat([torch.ones(len(relidx)), torch.zeros(remain_length)]).to(device)

            eot_indices.append(self.context_length - remain_length - 1)
            padding_zeros = torch.zeros(remain_length, dtype=torch.long).to(device)
            token = torch.cat([relidx, padding_zeros])
            text_input = {"input_ids": token.unsqueeze(0).type(self.dtype),
                          "attention_mask": rel_mask.unsqueeze(0).type(self.dtype)}
            language_rel = self.language_backbone(text_input)
            token_embedding = language_rel["hidden"].squeeze(0)
            full_token_embedding = torch.cat([
                self.hoi_prefix, token_embedding], dim=0)
            all_token_embeddings.append(full_token_embedding)
            full_mask = torch.cat([torch.ones(self.prefix_length).to(device), rel_mask]).to(device)
            all_masks.append(full_mask.type(self.dtype))
        eot_indices = torch.as_tensor(eot_indices)
        all_embeddings = torch.stack(all_token_embeddings, dim=0)  #####tensor 25 77 768
        all_mask = torch.stack(all_masks, dim=0)
        return eot_indices,all_embeddings,all_mask
    ...
    def forward(self, 
        ...
        else:#############################tokenizer bert——直接编码
            # language embedding
            language_dict_features = {}
            if captions is not None:###############
                #print(captions[0])
                tokenized = self.tokenizer.batch_encode_plus(captions,
                                                            max_length=self.cfg.MODEL.LANGUAGE_BACKBONE.MAX_QUERY_LEN,####310
                ...
                if train_ov_relation:
                    tokenized_rel = self.tokenizer.batch_encode_plus(captions_rel,
                                                                max_length=130,
                ...
                tokenizer_input = {"input_ids": input_ids,
                                "attention_mask": tokenized.attention_mask}#4 310 4 310——test 补2
                if train_ov_relation:
                    tokenizer_input_rel = {"input_ids": tokenized_rel.input_ids,
                                    "attention_mask": tokenized_rel.attention_mask}

                    self.context_length = 77
                    self.prefix_length = 8
                    self.conjun_length = 2
                    self.dtype=input_ids.dtype

                    rel_caption = captions_rel[0].split('. ')
                    rel_caption_all=None
                    if captions_rel_all is not None:
                        rel_caption_all = captions_rel_all[0][1:]



                if self.cfg.MODEL.LANGUAGE_BACKBONE.FREEZE:#################
                    with torch.no_grad():
                        language_dict_features = self.language_backbone(tokenizer_input)##########bertencoder   freeze
                        if train_ov_relation:
                            language_dict_features_rel = self.language_backbone(tokenizer_input_rel)
                            #language_dict_features_rel = None
                            if use_CLIPtext:

                                text = [clip.tokenize([pred_object]) for pred_object in rel_caption]  # tokenize
                                ...
                                all_embeddings = text_features.to(torch.float32)

                            else:###################################
                                eot_indices,all_embeddings,all_mask=self.encode_rel_text(rel_caption,device)####R100 27.52
                                eot_indices_all, all_embeddings_all, all_mask_all=None,None,None
                                if captions_rel_all is not None:
                                    eot_indices_all, all_embeddings_all, all_mask_all = self.encode_rel_text(rel_caption_all, device)
        ...
        if 'vl' in self.cfg.MODEL.SWINT.VERSION:
            ...
            visual_features = self.backbone(images.tensors)##freeze
            ...
            if train_ov_relation:
                if use_CLIPtext:
                    proposals, proposal_losses, fused_visual_features = self.rpn(images, visual_features, targets,
                                                                                 ...
                                                                                 lang_net=self.language_backbone,
                                                                                 tokenize=self.tokenizer,
                                                                                 prefix=self.hoi_prefix,
                                                                                 use_CLIPtext=use_CLIPtext)
                else:
                    if self.training:
                        proposals, proposal_losses, fused_visual_features,train_results = self.rpn(images, visual_features, targets, language_dict_features, positive_map,
                                                  captions, swint_feature_c4,language_dict_features_rel,captions_rel=captions_rel,positive_map_label_to_token_rel=positive_map_label_to_token_rel,train_ov_relation=train_ov_relation,eot_indices=eot_indices,all_embeddings=all_embeddings,all_mask=all_mask,eot_indices_all=eot_indices_all,all_embeddings_all=all_embeddings_all,all_mask_all=all_mask_all,lang_net=self.language_backbone,tokenize=self.tokenizer,prefix=self.hoi_prefix,rel_caption_all=rel_caption_all,now_iter=now_iter)
                    else:
                        proposals, proposal_losses, fused_visual_features = self.rpn(images,visual_features,
                                                                                                    targets,
                                                                                                    language_dict_features,
                                                                                                    positive_map,
                                                                                                    captions,
                ...
                proposals, proposal_losses, fused_visual_features = self.rpn(images, visual_features, targets, language_dict_features, positive_map,
                                              captions, swint_feature_c4,captions_rel=captions_rel,positive_map_label_to_token_rel=positive_map_label_to_token_rel,train_ov_relation=train_ov_relation)
        ...
        if self.cfg.MODEL.RELATION_ON:#########false
