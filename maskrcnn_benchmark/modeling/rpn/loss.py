...
class ATSSLossComputation(torch.nn.Module):
    ...
    def prepare_targets(self, targets, anchors, tokenized=None, positive_map=None, proj_tokens=None,tokenized_rel=None):
        cls_labels = []
        reg_targets = []
        token_labels = []
        map_labels = []
        ...
        for im_i in range(len(targets)):#positive_map——label对应的token 9 310
            targets_per_im = targets[im_i]
            assert targets_per_im.mode == "xyxy"
            ...
            if "positive_map_for_od_labels" in targets_per_im.fields():#################
                od_label_of_token_per_im = targets_per_im.get_field("positive_map_for_od_labels")#无意义
            ...
            anchors_per_im = cat_boxlist(anchors[im_i])#anchor:每个feat中每一dim（h*w）对应的原图框坐标

            num_anchors_per_loc = len(self.cfg.MODEL.RPN.ASPECT_RATIOS) * self.cfg.MODEL.RPN.SCALES_PER_OCTAVE#1
            num_anchors_per_level = [len(anchors_per_level.bbox) for anchors_per_level in anchors[im_i]]
            ious = boxlist_iou(anchors_per_im, targets_per_im)#18159 9

            gt_cx = (bboxes_per_im[:, 2] + bboxes_per_im[:, 0]) / 2.0
            gt_cy = (bboxes_per_im[:, 3] + bboxes_per_im[:, 1]) / 2.0
            gt_points = torch.stack((gt_cx, gt_cy), dim=1)###中心坐标
            ...
            distances = (anchor_points[:, None, :] - gt_points[None, :, :]).pow(2).sum(-1).sqrt()#中心距离
            ...
            for level, anchors_per_level in enumerate(anchors[im_i]):
                ...
                topk = min(self.cfg.MODEL.ATSS.TOPK * num_anchors_per_loc, num_anchors_per_level[level])#9
                _, topk_idxs_per_level = distances_per_level.topk(topk, dim=0, largest=False)#返回最小值
                candidate_idxs.append(topk_idxs_per_level + star_idx)
                star_idx = end_idx
            candidate_idxs = torch.cat(candidate_idxs, dim=0)####寻找anchor里和gt配对的框——固定为5*9=45

            # Using the sum of mean and standard deviation as the IoU threshold to select final positive samples
            candidate_ious = ious[candidate_idxs, torch.arange(num_gt)]#45 9
            iou_mean_per_gt = candidate_ious.mean(0)
            iou_std_per_gt = candidate_ious.std(0)
            iou_thresh_per_gt = iou_mean_per_gt + iou_std_per_gt###mean+std 9
            is_pos = candidate_ious >= iou_thresh_per_gt[None, :]####IOU大于阈值

            # Limiting the final positive samples’ center to object
            anchor_num = anchors_cx_per_im.shape[0]
            for ng in range(num_gt):
                candidate_idxs[:, ng] += ng * anchor_num
            e_anchors_cx = anchors_cx_per_im.view(1, -1).expand(num_gt, anchor_num).contiguous().view(-1)#9*18159
            ...
            b = bboxes_per_im[:, 3] - e_anchors_cy[candidate_idxs].view(-1, num_gt)############全部待选box和gt框的偏差 45 9
            is_in_gts = torch.stack([l, t, r, b], dim=1).min(dim=1)[0] > 0.01####最小值大于0.01（在gt框内）_# 筛选出中心点位于对应的真实框内的，作为候选正样本
            is_pos = is_pos & is_in_gts

            # if an anchor box is assigned to multiple gts, the one with the highest IoU will be selected.#保证anchor只和一个gt配对
            ious_inf = torch.full_like(ious, -INF).t().contiguous().view(-1)
            index = candidate_idxs.view(-1)[is_pos.view(-1)]#positive-89
            ious_inf[index] = ious.t().contiguous().view(-1)[index]
            ious_inf = ious_inf.view(num_gt, -1).t()########anchor全部样本的iou

            anchors_to_gt_values, anchors_to_gt_indexs = ious_inf.max(dim=1)###配对idx
            # get positive anchors index from ATSS
            positive_index = [i[0].item() for i in torch.nonzero(anchors_to_gt_indexs)]
            cls_labels_per_im = labels_per_im[anchors_to_gt_indexs]###anchor的标签
            cls_labels_per_im[anchors_to_gt_values == -INF] = 0

            if positive_map is not None:##################
                token_labels_per_im = token_per_im[anchors_to_gt_indexs]#18159 310
                ...
                unmatched_labels[-1] = 1  # token: none object - > 256######不匹配的label token-未配对的proposals
                token_labels_per_im[anchors_to_gt_values == -INF] = unmatched_labels
                # move from cpu to gpu
                token_labels_per_im = token_labels_per_im.to(cls_labels_per_im.device)#20267 310
            ...
            matched_gts = bboxes_per_im[anchors_to_gt_indexs]####为0的全都match到第一个box， reg_targets未置零——后续会处理，无关

            reg_targets_per_im = self.box_coder.encode(matched_gts, anchors_per_im.bbox)#match结果  相对位移
    ...
    def __call__(self, box_cls, box_regression, centerness, targets, anchors,
        ...
        if captions is not None:
            ...
            if self.cfg.MODEL.LANGUAGE_BACKBONE.TOKENIZER_TYPE == "clip":
                ...
                if train_ov_relation:
                    tokenized_rel = self.tokenizer.batch_encode_plus(captions_rel, padding="longest", return_tensors="pt")
                else:
                    tokenized_rel=None
        ...
        labels, reg_targets, token_labels, map_labels, gold_box_od_labels, od_label_of_tokens_labels, positive_indices = self.prepare_targets(targets, anchors,
                                                                             ...
                                                                             positive_map,
                                                                             proj_tokens
                                                                             )#labels 20267 token labels 20267 310
        ...
        if dot_product_logits is not None:####token(直接编码caption)和h*w的attention【图像级】
        ...
        reg_targets_flatten = torch.cat(reg_targets, dim=0)#18159*4 4
        ...
        #cls_loss = self.cls_loss_func(box_cls_flatten, labels_flatten.int()) / num_pos_avg_per_gpu#######cls_logits计算
        cls_loss=None
        ...
        if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.USE_DOT_PRODUCT_TOKEN_LOSS:#######整体预测logit对齐gt token_labels_stacked——gt的token label整合
            dot_product_token_loss = self.token_loss_func(dot_product_logits,
                                                          token_labels_stacked, text_masks=text_masks,######token_labels
                                                          version="binary") / num_pos_avg_per_gpu
        ...
        centerness_flatten = centerness_flatten[pos_inds]###########仅匹配positive，无问题
        ...
        return labels,token_labels, cls_loss, reg_loss * self.cfg.MODEL.ATSS.REG_LOSS_WEIGHT, centerness_loss, \
               token_logits_loss, \
