...
class ATSSPostProcessor(torch.nn.Module):
    ...
    def forward_for_single_feature_map(self, box_regression, centerness, anchors,
                                       box_cls=None, # score from obj classification head
                                       token_logits=None,
        ...
        A = box_regression.size(1) // 4#1

        if box_cls is not None:#################
            C = box_cls.size(1) // A

        if token_logits is not None:#false
        ...
        if box_cls is not None:###############
            ...
            box_cls = permute_and_flatten(box_cls, N, A, C, H, W)
            box_cls = box_cls.sigmoid()####B h*w 150
        ...
        if dot_product_logits is not None:#############
            ...
            else:################返回依据token标签的logit
                scores = convert_grounding_to_od_logits(logits=dot_product_logits, box_cls=box_cls,
                                                        positive_map=positive_map,
                                                        score_agg=self.score_agg)#mean
            box_cls = scores#与训练一样，也是每个anchor的score

        box_regression = permute_and_flatten(box_regression, N, A, 4, H, W)
        box_regression = box_regression.reshape(N, -1, 4)

        candidate_inds = box_cls > self.pre_nms_thresh#0.05
        pre_nms_top_n = candidate_inds.reshape(N, -1).sum(1)#超过阈值的logit项之和
        pre_nms_top_n = pre_nms_top_n.clamp(max=self.pre_nms_top_n)#1000

        centerness = permute_and_flatten(centerness, N, A, 1, H, W)
        centerness = centerness.reshape(N, -1).sigmoid()

        if fused_visual_features is not None:#############
            if fused_visual_features.shape[:2] == centerness.shape:#################
                box_feats = fused_visual_features
            else:
                ch = fused_visual_features.shape[1]
                box_feats = permute_and_flatten(fused_visual_features, N, A, ch, H, W)
        ...
        for idx, (per_box_cls, per_box_regression, per_pre_nms_top_n, per_candidate_inds, per_anchors) \
                in enumerate(zip(box_cls, box_regression, pre_nms_top_n, candidate_inds, anchors)):
            per_box_cls = per_box_cls[per_candidate_inds]

            per_box_cls, top_k_indices = per_box_cls.topk(per_pre_nms_top_n, sorted=False)

            per_candidate_nonzeros = per_candidate_inds.nonzero()[top_k_indices, :]####topk的proposal 排序

            per_box_loc = per_candidate_nonzeros[:, 0]#proposal idx(可能有重复)
            per_class = per_candidate_nonzeros[:, 1] + 1
            ...
            boxlist = BoxList(detections, per_anchors.size, mode="xyxy")
            boxlist.add_field("labels", per_class)
            boxlist.add_field("scores", torch.sqrt(per_box_cls))#开根号
            if fused_visual_features is not None:
                boxlist.add_field('box_features', box_feats[idx, per_box_loc, :])
            boxlist.add_field('per_box_loc', per_box_loc)

            boxlist = boxlist.clip_to_image(remove_empty=False)#####保证不超过图片边界
            boxlist = remove_small_boxes(boxlist, self.min_size)####删除空框
            results.append(boxlist)
    ...
    def forward(self, box_regression, centerness, anchors,
        ...
        for idx, (b, c, a) in enumerate(zip(box_regression, centerness, anchors)):
            ...
            feats = None if fused_visual_features is None else fused_visual_features[idx]
            sampled_boxes.append(
                self.forward_for_single_feature_map(b, c, a, o, t, d, positive_map, feats,positive_map_label_to_token_rel=positive_map_label_to_token_rel)
            )

        boxlists = list(zip(*sampled_boxes))
        boxlists = [cat_boxlist(boxlist) for boxlist in boxlists]
        if not (self.bbox_aug_enabled and not self.bbox_aug_vote):####################
            boxlists = self.select_over_all_levels(boxlists, sgg_mode, targets)
    ...
    # TODO: solve this issue in the future. No good solution
    # right now.
    def select_over_all_levels(self, boxlists, sgg_mode=None, targets=None):
        num_images = len(boxlists)
        results = []
        for i in range(num_images):
            # # remove duplicate box. turn off especially for overlapped classes !!!
            # if remove_duplicate_box:
            #     cls_scores = result.get_field("scores")
            #     pre_box_loc = result.get_field("per_box_loc")
            #     for box_id in pre_box_loc.unique():
            #         same_box_mask = (pre_box_loc == box_id)
            #         if same_box_mask.sum() > 1:
            #             max_score = cls_scores[same_box_mask].max()
            #             to_remove_mask = (cls_scores != max_score) & same_box_mask
            #             cls_scores[to_remove_mask] = -1

            if sgg_mode in ['sgcls', 'predcls']:
                # given ground-truth boxes, i.e., SGCls and PredCls mode
                assert targets is not None
                result = boxlists[i]
                img_target = targets[i]

                ious = boxlist_iou(result, img_target)
                label_match = result.get_field('labels').unsqueeze(1) == img_target.get_field('labels').unsqueeze(0)#996 2
                ious[~label_match] = -1.
                result.get_field("scores")[:] = 1###全为1
                match_scores = ious + result.get_field("scores").unsqueeze(-1)####996 2

                _, keep = match_scores.max(dim=0)
                result = result[keep]
            else:
                # multiclass nms
                result = boxlist_ml_nms(boxlists[i], self.nms_thresh)
                number_of_detections = len(result)
                if number_of_detections > self.fpn_post_nms_top_n > 0:#36
                    # Limit to max_per_image detections **over all classes**
                    cls_scores = result.get_field("scores")
                    image_thresh, _ = torch.kthvalue(
                    ...
                    keep = cls_scores >= max(0, image_thresh.item())#取前36个
                    keep = torch.nonzero(keep).squeeze(1)
                    result = result[keep]
            results.append(result)
        return results
    ...
    if positive_map is not None:
        ...
        if score_agg == "MEAN":######################
            for label_j in positive_map:
                # corner case
                if label_j > scores.shape[-1] or len(positive_map[label_j]) == 0:
                    print("error:",positive_map)
                    break
                scores[:, :, label_j - 1] = logits[:, :, torch.LongTensor(positive_map[label_j])].mean(-1)
        elif score_agg == "MAX":
            # torch.max() returns (values, indices)s
            for label_j in positive_map:
