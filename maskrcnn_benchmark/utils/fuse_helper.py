...
class BiMultiHeadAttention(nn.Module):
    ...
    def _shape(self, tensor: torch.Tensor, seq_len: int, bsz: int,single_feat=False):
        if single_feat:
    ...
    def forward(self, v, l, attention_mask_l=None,single_feat=False):
        if single_feat:
            tgt_len, embed_dim = v.size()
            bsz=1
            v=v.unsqueeze(0)
            l=l.unsqueeze(0)
            attention_mask_l=attention_mask_l.unsqueeze(0)
        else:
...
class BiAttentionBlockForCheckpoint(nn.Module):
    ...
    def forward(self, q0, q1=None, q2=None, q3=None, q4=None, l=None, attention_mask_l=None, dummy_tensor=None,single_feat=False):
        ...
        if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.SEPARATE_BIDIRECTIONAL:
            ...
            new_v, new_l = self.single_attention_call(visual_features_flatten, l, attention_mask_l=attention_mask_l,single_feat=single_feat)
            # [bs, N, C] -> [bs, C, N]
            if not single_feat:
                new_v = new_v.transpose(1, 2).contiguous()

            if single_feat:
                visu_feat=[new_v, None, None, None, None]
            else:
                start = 0
    ...
    def single_attention_call(self, v, l, attention_mask_l=None, dummy_tensor=None,single_feat=False):
        v = self.layer_norm_v(v)
        l = self.layer_norm_l(l)
        delta_v, delta_l = self.attn(v, l, attention_mask_l=attention_mask_l,single_feat=single_feat)
        # v, l = v + delta_v, l + delta_l
        v = v + self.drop_path(self.gamma_v * delta_v)############wv+v
        l = l + self.drop_path(self.gamma_l * delta_l)
        return v, l
