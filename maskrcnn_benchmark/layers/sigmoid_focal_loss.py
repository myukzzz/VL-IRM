...
def token_sigmoid_binary_focal_loss(pred_logits, targets, alpha, gamma, text_mask=None):
    ...
    if targets.dim() == 3:
        ...
        if text_mask is not None:
            ...
            targets = torch.masked_select(targets, text_mask)#1-d 16680*4*215    bg为全0
    else:
        bs, _ = pred_logits.shape
        if text_mask is not None:
            text_mask = (text_mask > 0).unsqueeze(0)
            text_mask = text_mask.repeat(pred_logits.size(0), 1)  # copy along the image channel dimension
            pred_logits = torch.masked_select(pred_logits, text_mask)
            targets = torch.masked_select(targets, text_mask)#1-d 16680*4*215

        # print(pred_logits.shape)
        # print(targets.shape)

    p = torch.sigmoid(pred_logits)#sigmoid——二分类回归
    ce_loss = F.binary_cross_entropy_with_logits(pred_logits, targets, reduction="none")#sigmoid+bce
